	...
	try {
		...
		if(!isset($name) || trim($name) == ''){
		   echo "empty_name";
			...
			$stmt2 = $conn->prepare("SELECT tc.title FROM theory_content AS tc INNER JOIN theory_topic AS t ON tc.id = t.id_theory_content WHERE t.id_topic = :idtopic");
			...
			if ($total2 > 0){
				while ($row2 = $stmt2->fetchObject()) {
					...
					$miscursos[$punter]['name'] = $row2->title;
